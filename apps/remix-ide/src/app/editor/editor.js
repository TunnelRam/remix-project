...
class Editor extends Plugin {
  ...
  render () {
    ...
    this.el.setCurrentContent = (value) => {
      if (this.session[this.currentFile]) this.sessions[this.currentFile].setValue(value)
    }
    this.renderComponent()
  ...
  renderComponent () {
    ReactDOM.render(
      <EditorUI editorAPI={this.api} theme={this.currentTheme} currentFile={this.currentFile} sourceAnnotationsPerFile={this.sourceAnnotationsPerFile} markerPerFile={this.markerPerFile} onBreakPointAdded={this.onBreakPointAdded} onBreakPointCleared={this.onBreakPointCleared} onDidChangeContent={this.onDidChangeContent} />
      , this.el)
  }
  ...
  addDecoration (decoration, filePath, typeOfDecoration) {
    if (filePath && !this.sessions[filePath]) throw new Error('file not found' + filePath)
    const path = filePath || this.currentFile
  ...
  addAnnotation (annotation, filePath) {
    this.addDecoration(annotation, filePath, 'sourceAnnotationsPerFile')
  }

  highlight (position, filePath) {
    this.addDecoration({ position }, filePath, 'markerPerFile')
  }
